---
layout: default
title: AI Hub Challenge References
parent: References
nav_order: 3
tags: [references, ai-hub, architecture, documentation]
permalink: /references/ai-hub/
---

# AI Hub Challenge References

This page contains consolidated reference materials for the AI Hub Challenge.

## Architecture Resources

### Azure AI Hub Reference Architecture
- [Azure OpenAI baseline Landing Zone reference architecture](https://learn.microsoft.com/azure/architecture/ai-ml/architecture/azure-openai-baseline-landing-zone)
- [Azure AI Hub Gateway Solution Accelerator](https://github.com/Azure-Samples/ai-hub-gateway-solution-accelerator/tree/main)
- [Scaling AI workloads across the enterprise](https://learn.microsoft.com/azure/cloud-adoption-framework/scenarios/ai/ai-ready/scale-ai-workloads)

### Network Security & Private Link
- [Private networking for Azure AI services](https://learn.microsoft.com/azure/ai-services/cognitive-services-virtual-networks)
- [Azure OpenAI Service Private Link Support](https://learn.microsoft.com/azure/ai-services/openai/how-to/private-link)
- [Azure API Management in VNET](https://learn.microsoft.com/azure/api-management/api-management-using-with-vnet)
- [Azure Private Link for Azure Key Vault](https://learn.microsoft.com/azure/key-vault/general/private-link-service)

### Framework & Guidance
- [Microsoft Cloud Adoption Framework - AI Scenario](https://learn.microsoft.com/azure/cloud-adoption-framework/scenarios/ai/)

## Return to Challenge
- [AI Hub Challenge Overview](../03-aihub/ifs-aihub-overview.md)
- [Step 1 - Scenario](../03-aihub/ifs-aihub-step1-scenario.md)
- [Step 2 - Requirements](../03-aihub/ifs-aihub-step2-requirements.md)
- [Step 3 - Design](../03-aihub/ifs-aihub-step3-design.md)
- [Step 4 - Present & Justify](../03-aihub/ifs-aihub-step4-present.md)
